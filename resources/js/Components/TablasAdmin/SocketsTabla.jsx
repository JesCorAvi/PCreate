...
export default function ArticulosTabla() {
    ...
    useEffect(() => {
        getSockets(currentPage);
    }, [currentPage]); // Se vuelve a cargar cuando cambia la página actual

    function getSockets(page) {
        axios.post(route('socket.getSockets', { page: page }))
            .then((response) => {
    ...
    return (
        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            href={route('socket.create')} method="get" as="button"> Crear Socket </Link>
                    ...
                    {sockets.map((socket) => (
                        <tr key={socket.id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 flex gap-2">
                                <Link className='inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150' href={route('socket.edit', socket.id)} method="get" as="button"> Editar </Link>
                                <DangerButton text="Borrar" onClick={() => delSockets(socket.id)}></DangerButton>
                            </td>
                        </tr>
                    ))}
                </tbody>
