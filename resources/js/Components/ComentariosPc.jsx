...
export default function ComentariosPc({ user, id, onComentarioCreado, onComentarioBorrado }) {
    ...
    const [userHasCommented, setUserHasCommented] = useState(user ? user.comentarios.some(comentario =>
        comentario.comentable_type === 'App\\Models\\Pc' && comentario.comentable_id === id
    ) : false);
    ...
    function getComentariosWhere(page) {
        axios.post(route('comentario.getComentariosWhere', { page: page, id: id, type: 'Pc', commentableId: id }))
            .then((response) => {
    ...
    function createComentario(e) {
        e.preventDefault();
        axios.post(route('comentario.store', { commentableType: 'Pc', commentableId: id }), {
    ...
    return (
        <section className="flex-1 flex flex-col text-justify md:px-10 xl:px-60">
